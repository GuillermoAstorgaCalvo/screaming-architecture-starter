/**
 * PasswordInputMessages Component Tests
 *
 * Tests for the PasswordInputMessages component including:
...
const HELPER_TEXT = 'Must be at least 8 characters';

describe('PasswordInputMessages - Rendering', () => {
	it('returns null when neither error nor helperText is provided', () => {
		const { container } = renderWithProviders(<PasswordInputMessages inputId={INPUT_ID} />);
		expect(container.firstChild).toBeNull();
	});

	it('returns null when error is empty string and helperText is not provided', () => {
		const { container } = renderWithProviders(
			<PasswordInputMessages inputId={INPUT_ID} error="" />
		);
		expect(container.firstChild).toBeNull();
	});
	...
	it('returns null when helperText is empty string and error is not provided', () => {
		const { container } = renderWithProviders(
			<PasswordInputMessages inputId={INPUT_ID} helperText="" />
		);
		expect(container.firstChild).toBeNull();
	});

	it('renders error message when error is provided', () => {
		renderWithProviders(<PasswordInputMessages inputId={INPUT_ID} error={ERROR_MESSAGE} />);
		expect(screen.getByText(ERROR_MESSAGE)).toBeInTheDocument();
	});

	it('renders helper text when helperText is provided', () => {
		renderWithProviders(<PasswordInputMessages inputId={INPUT_ID} helperText={HELPER_TEXT} />);
		expect(screen.getByText(HELPER_TEXT)).toBeInTheDocument();
	});

	it('renders both error and helper text when both are provided', () => {
		renderWithProviders(
			<PasswordInputMessages inputId={INPUT_ID} error={ERROR_MESSAGE} helperText={HELPER_TEXT} />
		);
		expect(screen.getByText(ERROR_MESSAGE)).toBeInTheDocument();
		expect(screen.getByText(HELPER_TEXT)).toBeInTheDocument();
	});
...
describe('PasswordInputMessages - ID Generation', () => {
	...
	it('handles different inputId values', () => {
		const customId = 'custom-password-input';
		renderWithProviders(<PasswordInputMessages inputId={customId} error={ERROR_MESSAGE} />);
		const errorElement = screen.getByText(ERROR_MESSAGE);
		expect(errorElement).toHaveAttribute('id', `${customId}-error`);
	});
});

describe('PasswordInputMessages - Accessibility', () => {
...
describe('PasswordInputMessages - Error and Helper Text Interaction', () => {
...
describe('PasswordInputMessages - Edge Cases', () => {
	it('handles falsy error values correctly', () => {
		const { container: container1 } = renderWithProviders(
			<PasswordInputMessages inputId={INPUT_ID} error={undefined} />
		);
		expect(container1.firstChild).toBeNull();

		const { container: container2 } = renderWithProviders(
			<PasswordInputMessages inputId={INPUT_ID} error={null as unknown as string} />
		);
	...
	it('handles falsy helperText values correctly', () => {
		const { container: container1 } = renderWithProviders(
			<PasswordInputMessages inputId={INPUT_ID} helperText={undefined} />
		);
		expect(container1.firstChild).toBeNull();

		const { container: container2 } = renderWithProviders(
			<PasswordInputMessages inputId={INPUT_ID} helperText={null as unknown as string} />
		);
		expect(container2.firstChild).toBeNull();
	});

	it('handles whitespace-only error as truthy', () => {
		renderWithProviders(<PasswordInputMessages inputId={INPUT_ID} error="   " />);
	...
	it('handles whitespace-only helperText as truthy', () => {
		renderWithProviders(<PasswordInputMessages inputId={INPUT_ID} helperText="   " />);
